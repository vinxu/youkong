...
func main() {
	...
	provider := flag.String("provider", "qwen", "提供商: qwen 或 kimi")
	...
	// 从环境变量获取 API Key（如果未通过参数提供）
	if *qwenKey == "" {
		*qwenKey = os.Getenv("LLM_API_KEY")
	}
	if *kimiKey == "" {
	...
	// 创建测试服务
	testService := service.NewModelTestService(*qwenKey, *kimiKey)
...
func printResult(r *service.TestResult) {
	fmt.Printf("模型: %s (%s)\n", r.Model, r.Provider)
	fmt.Printf("响应时间: %dms\n", r.TotalMs)
