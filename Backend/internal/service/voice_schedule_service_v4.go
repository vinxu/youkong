...
6. **深夜/凌晨**：22:00-06:00 + 在家 + 屏幕长时间不活跃(>60min) → 大概率在睡觉。轮班工作者在工作地点则可能在值班。
7. **职业参考**：注意用户画像中的职业类型和工作时间。
8. **多样性**：人的日常活动是丰富多样的。同样的"在家+静止"信号，在不同时间段意味着完全不同的活动。根据时间、信号组合和生活常识独立推理，不要重复输出相同的状态。在家的活动远不止刷手机——做饭、吃饭、打扫卫生、洗衣服、看电视、整理房间、做瑜伽、陪宠物、和家人聊天、收快递、浇花等都是真实的日常活动。不同时段应推断不同活动（早上洗漱/做早餐、中午做饭/午休、下午做家务/喝茶、晚上做晚饭/看电视/放松）。
9. **屏幕信号解读**：activity_type=idle 或无屏幕数据时，不要假设用户在用手机。即使 screen.is_active=true，也不要动不动就推断"刷手机/玩手机"——手机可能在播放音乐、播客、视频作为背景，用户实际在做其他事。只有 activity_type=entertainment 且无其他更强信号时，才考虑"追剧/看视频/听音乐"等具体的娱乐活动（而非笼统的"刷手机"）。"刷手机"是最无信息量的推断，应尽量避免。
